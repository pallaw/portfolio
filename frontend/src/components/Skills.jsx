        ...
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {skillCategories.map((category, index) => (
              <Card
                key={index}
                className="bg-gray-900/50 border-gray-800 hover:border-accent-jetbrains/50 transition-all duration-300 p-6 space-y-4"
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {category.items.map((skill, idx) => (
                    <Badge
                      key={idx}
                      className="bg-accent-jetbrains/10 text-accent-jetbrains border-accent-jetbrains/30 hover:bg-accent-jetbrains/20 font-mono text-sm py-1.5 px-3 hover:scale-105 transition-transform cursor-default"
                    >
                      {skill}
                    </Badge>
                  ))}
